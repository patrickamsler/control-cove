    ...
    devices.forEach((device, index) => {
      if (seen.has(device.id)) {
        ctx.addIssue({
          code: "custom",
          path: [index],
          message: `Duplicate ${label} id: ${device.id}`,
        });
      }
      seen.add(device.id);
    });
...
const describeIssue = (
  ...
  const device =
    Array.isArray(raw) && typeof index === "number"
...
const parse = <T extends { id: number }>(
  ...
  const result = z
    .array(schema)
    .superRefine(uniqueIds<T>(label))
    .safeParse(raw);
  if (!result.success) {
    const details = result.error.issues.map((issue) =>
      describeIssue(raw, issue, label),
    );
...
export const switches: SwitchConfig[] = parse(
  switchConfigSchema,
  switchConfig,
  "switch",
);
export const sensors: SensorConfig[] = parse(
  sensorConfigSchema,
  sensorConfig,
  "sensor",
);
