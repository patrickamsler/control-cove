...
const validSwitches = [
  {
    id: 1,
    name: "Kitchen",
    commandTopic: "shellies/kitchen/relay/0/command",
    stateTopic: "shellies/kitchen/relay/0",
  },
  {
    id: 2,
    name: "Bedroom",
    commandTopic: "shellies/bedroom/relay/0/command",
    stateTopic: "shellies/bedroom/relay/0",
  },
];
...
const loadDevices = (
  switchConfig: unknown = validSwitches,
  sensorConfig: unknown = validSensors,
) => {
...
describe("devices config validation", () => {
  it("rejects duplicate switch ids", async () => {
    await expect(
      loadDevices([...validSwitches, { ...validSwitches[0], name: "Clone" }]),
    ).rejects.toThrow("Duplicate switch id: 1");
  });

  it("rejects duplicate sensor ids", async () => {
    await expect(
      loadDevices(validSwitches, [
        ...validSensors,
        { ...validSensors[0], name: "Clone" },
      ]),
    ).rejects.toThrow("Duplicate sensor id: 1");
  });

  it.each(["commandTopic", "stateTopic"])(
    "rejects a switch with an empty %s",
    async (field) => {
      await expect(
        loadDevices([{ ...validSwitches[0], [field]: "" }]),
      ).rejects.toThrow(`switch "Kitchen" (id 1) ${field}`);
    },
  );

  it.each(["commandTopic", "stateTopic"])(
    "rejects a switch with a whitespace-only %s",
    async (field) => {
      await expect(
        loadDevices([{ ...validSwitches[0], [field]: "   " }]),
      ).rejects.toThrow(`switch "Kitchen" (id 1) ${field}`);
  ...
  it.each(["commandTopic", "stateTopic"])(
    "rejects a switch with a missing %s",
    async (field) => {
      const { [field]: _omitted, ...incomplete } = validSwitches[0] as Record<
        string,
        unknown
      >;

      await expect(loadDevices([incomplete])).rejects.toThrow(
  ...
  it("rejects a sensor with an empty statusTopic", async () => {
    await expect(
      loadDevices(validSwitches, [{ ...validSensors[0], statusTopic: "" }]),
    ).rejects.toThrow('sensor "Livingroom" (id 1) statusTopic');
  });

  it("rejects a sensor with a non-string statusTopic", async () => {
    await expect(
      loadDevices(validSwitches, [{ ...validSensors[0], statusTopic: 42 }]),
    ).rejects.toThrow('sensor "Livingroom" (id 1) statusTopic');
  });

  it("rejects a switch with a non-integer id", async () => {
    await expect(
      loadDevices([{ ...validSwitches[0], id: 1.5 }]),
    ).rejects.toThrow('switch "Kitchen" (id 1.5) id');
  });
});
