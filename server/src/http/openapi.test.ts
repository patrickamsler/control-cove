...
describe("the OpenAPI document", () => {
  ...
  it("describes each response as an array of the matching schema", () => {
    const responseSchema = (path: "/api/switches" | "/api/sensors") =>
      openApiDocument.paths[path].get.responses["200"].content[
        "application/json"
      ].schema;

    expect(responseSchema("/api/switches").items.$ref).toBe(
      "#/components/schemas/Switch",
    );
    expect(responseSchema("/api/sensors").items.$ref).toBe(
