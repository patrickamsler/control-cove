...
const setup = () => {
  ...
  const resourceReaders: Record<
    string,
    (uri: URL, variables: Record<string, string>) => Promise<unknown>
  > = {};
  ...
  return {
    ...
    call: (name: string, args: unknown = {}) =>
      toolHandlers[name](args as never),
  };
};
...
describe("the resources", () => {
  ...
  it("reads a single switch from its template uri", async () => {
    const result = (await ctx.resourceReaders["switch"](
      new URL("switch://10"),
      { id: "10" },
    )) as {
  ...
  it("throws for an unknown template id", async () => {
    await expect(
      ctx.resourceReaders["sensor"](new URL("sensor://999"), { id: "999" }),
    ).rejects.toThrow("No sensor with id 999");
  });
});
...
const listenerCount = (deviceService: DeviceService): number =>
  (
    deviceService as unknown as {
      events: { listenerCount(name: string): number };
    }
  ).events.listenerCount("switch");
