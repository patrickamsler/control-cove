...
export function registerMcpHandlers(
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      ...
      return found
        ? ok(found)
        : fail(
            `No switch with id ${id}. Call list_switches to see the available ids.`,
          );
    },
  );
  ...
  server.registerTool(
    ...
    async ({ id, state }) => {
      ...
      logger.info(
        `[MCP] Switch ${id} (${target.name}) confirmed ${state ? "on" : "off"}`,
      );
      return ok(result);
    },
  );
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      ...
      if (!found) {
        return fail(
          `No sensor with id ${id}. Call list_sensors to see the available ids.`,
        );
      }
  ...
  server.registerResource(
    ...
    "devices://all",
    {
      title: "All devices",
      description:
        "Every configured switch and sensor with its latest known state, as one document.",
  ...
  server.registerResource(
    ...
    {
      title: "Switch",
      description: "One light switch and its latest known state.",
      mimeType: "application/json",
    },
    async (uri, { id }) => {
  ...
  server.registerResource(
    ...
    {
      title: "Sensor",
      description: "One environment sensor and its latest reading.",
      mimeType: "application/json",
    },
    async (uri, { id }) => {
