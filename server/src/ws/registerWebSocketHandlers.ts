...
export function registerWebSocketHandlers(
  ...
  deviceService.onSwitchChanged((data) =>
    webSocketService.emit("switch", data),
  );
  deviceService.onSensorChanged((data) =>
    webSocketService.emit("sensor", data),
  );
  ...
  webSocketService.onEvent("updateSwitch", (payload) => {
    ...
    if (!update.success) {
      logger.error(
        `Ignoring malformed updateSwitch payload: ${JSON.stringify(payload)}`,
      );
      return;
    }
