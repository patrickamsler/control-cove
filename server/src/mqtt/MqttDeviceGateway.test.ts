...
const setup = () => {
  ...
  const mqttClient = {
    ...
    subscribeToTopic: vi.fn(
      (topic: string, onMessage: (message: string) => void) => {
  ...
  return {
    ...
    publish: (topic: string, message: string) =>
      topicHandlers.get(topic)!(message),
    topicHandlers,
  };
};
...
describe("MqttDeviceGateway sensor payloads", () => {
  ...
  it("keeps the reported device_id server-internal, off the DTO", () => {
    ctx.publish(
      "devices/livingroom/sensors/status",
      '{"device_id":"A001","temperature":20,"humidity":40}',
    );
  ...
  it("falls back to the topic when the device reports no device_id", () => {
    ...
    expect(ctx.deviceService.getSensorReading(1)?.deviceId).toBe(
      "devices/livingroom/sensors/status",
    );
  });
  ...
  it("keeps a valid reading when the device_id is not a string", () => {
    ctx.publish(
      "devices/livingroom/sensors/status",
      '{"device_id":42,"temperature":20,"humidity":40}',
    );

    expect(ctx.deviceService.getSensorReading(1)).toEqual({
      temperature: 20,
      humidity: 40,
      deviceId: "devices/livingroom/sensors/status",
    });
  });
  ...
  it.each([
    ...
    expect(() =>
      ctx.publish("devices/livingroom/sensors/status", message),
    ).not.toThrow();
...
describe("MqttDeviceGateway switch payloads", () => {
  it.each([
    ...
    expect(ctx.deviceService.getSwitch(10)).toEqual({
      id: 10,
      name: "Test Lamp",
      state: expected,
    });
...
describe("MqttDeviceGateway.sendSwitchCommand", () => {
  it('publishes "on" to the command topic of the addressed switch', () => {
    ...
    expect(ctx.publishMessage).toHaveBeenCalledExactlyOnceWith(
      "shellies/lamp/relay/0/command",
      "on",
    );
  });
