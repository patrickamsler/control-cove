...
class FakeMqttClient extends EventEmitter {
  connected = false;
  publish = vi.fn(
    (_topic: string, _message: string, callback?: (err?: Error) => void) =>
      callback?.(),
  );
  subscribe = vi.fn((_topic: string, callback?: (err?: Error) => void) =>
    callback?.(),
  );
  unsubscribe = vi.fn((_topic: string, callback?: (err?: Error) => void) =>
    callback?.(),
  );
  end = vi.fn(() => {
...
beforeEach(() => {
  ...
  connectMock.mockReturnValue(
    client as unknown as ReturnType<typeof mqtt.connect>,
  );
  service = new MqttClient();
});
...
describe("MqttClient.connectToBroker", () => {
  it.each(["MQTT_USERNAME", "MQTT_PASSWORD"])(
    "throws when %s is missing",
    (variable) => {
      vi.stubEnv(variable, "");

      expect(() => service.connectToBroker()).toThrow(
        "Broker username and password are required",
      );
  ...
  it("connects to the configured url with the configured credentials", () => {
    ...
    expect(connectMock).toHaveBeenCalledExactlyOnceWith(
      "mqtt://localhost:1883",
      {
        username: "test-user",
...
describe("MqttClient reconnect behaviour", () => {
...
describe("MqttClient subscriptions", () => {
  beforeEach(() => {
    service.connectToBroker();
    client.goOnline();
  });
  ...
  it("ignores messages on topics without a listener", () => {
    ...
    expect(() =>
      client.emit("message", "topic/unknown", Buffer.from("hello")),
    ).not.toThrow();
...
describe("MqttClient publishing", () => {
  ...
  it("reports a publish failure without throwing", () => {
    service.connectToBroker();
    client.goOnline();
    client.publish.mockImplementationOnce((_topic, _message, callback) =>
...
describe("MqttClient.disconnectFromBroker", () => {
